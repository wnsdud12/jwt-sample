...
6. `http://localhost:5173/admin/login`에서 관리자 로그인 (백엔드 구현 후 사용 가능)
7. 관리자·사용자를 동시에 다른 탭에서 로그인해도 세션 충돌 없음 확인
...
| Method | Path | 인증 | 설명 |
|--------|------|------|------|
| POST | `/api/admin/auth/login` | 공개 | 관리자 로그인 → accessToken(JSON) + admin refresh Cookie |
| POST | `/api/admin/auth/refresh` | Cookie | 관리자 accessToken 재발급 |
| POST | `/api/admin/auth/logout` | Cookie | 관리자 Cookie 만료 |
| GET | `/api/admin/users` | Bearer JWT (ADMIN) | 전체 회원 목록 |
...
사용자(`/`)와 관리자(`/admin`)가 동일 도메인을 사용할 때, 한 브라우저에서 동시에 로그인해도 세션이 충돌하지 않도록 이중으로 분리한다.

| 분리 방식 | 사용자 | 관리자 |
|-----------|--------|--------|
| Cookie 이름 | `refresh_token` | `admin_refresh_token` |
| Cookie Path | `/api/auth` | `/api/admin/auth` |
| Zustand 스토어 | `useAuthStore` | `useAdminAuthStore` |
| Axios 인스턴스 | `apiClient` | `adminApiClient` |
| Single-flight | `refreshPromise` | `adminRefreshPromise` |

브라우저는 Cookie를 **(name + domain + path) 조합**으로 식별한다.  
이름이 같아도 path가 다르면 브라우저는 이를 별개의 Cookie로 저장·관리한다.

**Path 분리가 핵심이다.** 브라우저는 요청 URL이 Cookie의 `path` 값으로 시작할 때만 해당 Cookie를 자동으로 첨부하므로, path만 달라도 쿠키 충돌을 막기에 충분하다.

```
user_refresh_token  (path=/api/auth)
admin_refresh_token (path=/api/admin/auth)

POST /api/auth/refresh       → user_refresh_token 만 전송
POST /api/admin/auth/refresh → admin_refresh_token 만 전송
GET  /api/users/me           → 둘 다 전송 안 됨 (경로 미해당)
```

**Name 분리는 Defense in Depth다.** path가 실수로 넓어졌을 때를 대비한 추가 안전망이다.  
단, 이미 라이브된 서비스에서 name을 변경하면 기존 사용자 전원이 로그아웃되므로, 운영 중인 서비스라면 path 분리만 적용해도 충돌 문제를 해결할 수 있다.
...
| 2. Origin/Referer 검증 | `POST /api/auth/refresh`, `POST /api/auth/logout` | `CookieCsrfDefenseFilter`에서 허용 Origin 화이트리스트(`app.cors.allowed-origins`)와 대조. Origin/Referer가 없는 경우 `Sec-Fetch-Site: same-origin`만 허용 (`same-site` 불허 — 서브도메인 탈취 시 우회 가능) |
| 3. 커스텀 헤더 요구 | 동일 엔드포인트 | `X-Requested-With: XMLHttpRequest` 필수 — 단순 `<form>` CSRF 차단 |

### SameSite 값 선택 기준

SameSite는 서비스 특성에 따라 결정한다.

| 값 | 동작 | 적합한 서비스 |
|----|------|--------------|
| `Strict` | 외부 링크 클릭 포함 모든 cross-site 요청에 Cookie 미전송. 외부 링크로 유입 시 로그인이 풀린다. | 금융·관리자 페이지 등 보안 우선 서비스 |
| `Lax` | 링크 클릭(GET 탐색)에는 Cookie 전송, cross-site POST form은 차단. 외부 링크 클릭 후에도 로그인 유지. | SNS·쇼핑몰 등 공유 링크 클릭 후 로그인 상태가 유지돼야 하는 서비스 |

`Lax`로 변경하더라도 2·3계층(Origin 검증 + `X-Requested-With`)이 cross-site POST를 추가로 막으므로 CSRF 방어는 유지된다.
...
무차별 대입 공격(brute-force)으로부터 계정을 보호하기 위해 IP 기반 요청 횟수를 제한합니다.

| 항목 | 값 |
|------|----|
| 적용 대상 | `POST /api/auth/login`, `POST /api/admin/auth/login` |
| 제한 | IP당 **1분에 최대 5회** |
| 초과 시 | `429 Too Many Requests` |
| 구현 | Redis `INCR` + TTL — 원자적 연산으로 카운터 관리 |

> IP 추출 순서: `X-Forwarded-For` 첫 번째 값 → `remoteAddr` (리버스 프록시 환경 대응)
...
`GlobalExceptionHandler`가 모든 예외를 일관된 JSON 형식으로 변환합니다.

| 예외 타입 | HTTP 상태 | 설명 |
|----------|-----------|------|
| `AuthException` | 예외 내 `ErrorCode` 참조 | 인증/인가 관련 예외 |
| `MethodArgumentNotValidException` | 400 | `@Valid` 검증 실패 |
| `Exception` (catch-all) | 500 | 예상치 못한 예외 — **서버 로그에만 상세 기록**, 클라이언트에는 일반 메시지만 반환 (내부 구조 노출 방지) |
